...
export default function PreviewPage() {
  ...
  useEffect(() => {
    ...
    storage.getById(id).then((site) => {
      ...
      setChatHistory(site.chat_history ?? []);
      if (site.page_name) {
  ...
  const saveChatHistory = useCallback(async (websiteId: string, history: ChatHistoryEntry[]) => {
    await storage.update(websiteId, { chat_history: history } as Partial<Website>);
  }, []);

  // ── Build conversation context for AI (only text, no source_code) ─
  const buildAIConversationHistory = (history: ChatHistoryEntry[]) => {
  ...
  const handleEditSubmit = async () => {
    ...
    const userMessage: ChatHistoryEntry = {
      ...
      prev_source_code: website.source_code,
    };
    ...
    const aiHistory = buildAIConversationHistory(chatHistory); // before user's new message
    ...
    await surgicalEditWebsite(
      ...
      editPrompt,
      {
        ...
        onDone: async (finalCode: string, patchCount: number) => {
          ...
          const patchDescriptions = patches.map(p => `${p.success ? '✓' : '✗'} ${p.description}`).join('\n');
          const aiMessage: ChatHistoryEntry = {
            id: `msg_${Date.now()}_${Math.random().toString(36).slice(2, 7)}`,
            role: 'assistant',
            content: patchCount > 0
              ? `Applied ${patchCount} change${patchCount !== 1 ? 's' : ''} (+${stats.added} / -${stats.removed} lines):\n${patchDescriptions}`
              : 'Applied edit via full rewrite.',
            created_at: new Date().toISOString(),
          };

          const finalHistory = [...updatedHistoryWithUser, aiMessage];
          setChatHistory(finalHistory);
          await saveChatHistory(website.id, finalHistory);
  ...
  const handleDeploy = async () => {
    ...
    const url = `https://${pageSlug}.alsytes.dev`;
    setDeployedUrl(url);
  ...
  const goToDeployed = () => {
    ...
    window.open(`https://${pageSlug}.alsytes.dev`, '_blank');
  };

  const clearChatHistory = async () => {
    if (!website) return;
    setChatHistory([]);
    await saveChatHistory(website.id, []);
  };
  ...
  if (notFound) {
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-base text-center px-6">
        ...
        >
          <ArrowLeft size={14} /> Back to Alsytes
        </button>
      </div>
    );
  }
        ...
        {!fullscreen && (
          ...
          >
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              {/* Chat history button — shows badge if history exists */}
              <button
              ...
              >
                ...
                {chatHistory.length > 0 && (
                  <span
                    ...
                    style={{ background: '#7C3AED', fontSize: '9px', fontWeight: 700 }}
                  >
      ...
      <div className="flex-1 flex overflow-hidden relative flex-col">
        ...
        {!isMobileView && (
          <div className="flex-1 flex overflow-hidden relative">
            {/* Iframe */}
            <div
              className={`flex-1 flex items-start justify-center overflow-auto bg-zinc-900 transition-all ${showCode || showChatPanel ? 'w-1/2' : 'w-full'}`}
            ...
            {/* Chat history panel (side) */}
            <AnimatePresence>
              {showChatPanel && !showCode && (
                <motion.div
                  ...
                  <>
                    ...
                    <div className="mb-5 px-3.5 py-3 rounded-xl bg-base-100 border border-surface-border">
                      ...
                      <p className="text-xs font-mono text-text-primary break-all leading-relaxed">
                        ...
                        <span className="text-text-muted">.alsytes.dev</span>
                      </p>
                    </div>
                    <button
...
function ChatHistoryPanel({ chatHistory, website, chatEndRef, onClear, onClose, onQuickEdit, isMobile }: ChatHistoryPanelProps) {
  const [showClearConfirm, setShowClearConfirm] = useState(false);

  return (
    <>
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-surface-border bg-base-100 flex-shrink-0">
        {isMobile && (
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <span className="text-xs font-semibold text-text-primary">Edit History</span>
          {chatHistory.length > 0 && (
            <span
              className="px-1.5 py-0.5 rounded-full text-xs font-semibold"
              style={{ background: 'rgba(124,58,237,0.12)', color: '#7C5CFC' }}
            >
              {Math.floor(chatHistory.filter(m => m.role === 'user').length)}
            </span>
          )}
        </div>
        <div className="flex items-center gap-2 mt-1">
      ...
      <AnimatePresence>
        {showClearConfirm && (
          <motion.div
            ...
            animate={{ height: 'auto', opacity: 1 }}
            exit={{ height: 0, opacity: 0 }}
            className="overflow-hidden flex-shrink-0 border-b border-surface-border"
          >
            <div className="px-4 py-3 flex items-center justify-between gap-3">
              <p className="text-xs text-text-muted">Clear all edit history for this project?</p>
              <div className="flex gap-2">
                <button
                  onClick={() => setShowClearConfirm(false)}
                  className="px-3 py-1.5 rounded-lg text-xs text-text-muted bg-base-200 hover:bg-base-300 transition-colors"
                >
                  Cancel
                </button>
                <button
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-3">
        {chatHistory.length === 0 ? (
          <div className="flex flex-col items-center justify-center flex-1 py-12 text-center">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center mb-3" style={{ background: 'rgba(124,58,237,0.08)', border: '1px solid rgba(124,58,237,0.15)' }}>
              <MessageSquare size={20} className="text-violet-400/60" />
            </div>
            <p className="text-sm font-semibold text-text-primary mb-1">No edit history yet</p>
            <p className="text-xs text-text-muted max-w-[200px] leading-relaxed">
              Use "Ask AI" to make edits — all changes will be tracked here
            </p>
            <button
              onClick={() => onQuickEdit('')}
              className="mt-4 flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-white transition-all hover:opacity-90"
              style={{ background: 'linear-gradient(135deg, #7C5CFC, #5A3DE8)' }}
            >
              <Wand2 size={11} /> Start editing
            </button>
          </div>
        ) : (
          <>
            ...
            >
              ...
              <span className="text-text-muted font-medium truncate">{website.name}</span>
              <span className="text-text-muted ml-auto flex-shrink-0 flex items-center gap-1">
                <Clock size={10} />
            ...
            {chatHistory.map((entry, i) => (
              <motion.div
                ...
                className={`flex ${entry.role === 'user' ? 'justify-end' : 'justify-start'} gap-2`}
              >
                ...
                <div className={`max-w-[85%] ${entry.role === 'user' ? 'order-last' : ''}`}>
                  <div
                    className="rounded-2xl px-3 py-2.5 text-xs leading-relaxed"
                    style={entry.role === 'user'
                      ? { background: 'linear-gradient(135deg, #7C3AED, #5A3DE8)', color: '#fff', borderBottomRightRadius: 6 }
                      : { background: '#fff', border: '1px solid #E2DFEF', color: '#4A4660', borderTopLeftRadius: 6 }
                    }
                  >
                    <pre className="whitespace-pre-wrap font-sans">{entry.content}</pre>
                  </div>
                  <div className="flex items-center gap-2 mt-1 px-1">
                    <span className="text-[10px]" style={{ color: '#9A96B0' }}>
                      {formatRelativeTime(entry.created_at)}
                    </span>
                    {entry.role === 'user' && (
                      <button
                        onClick={() => onQuickEdit(entry.content)}
      ...
      {chatHistory.length > 0 && (
        <div className="flex-shrink-0 px-4 py-3 border-t border-surface-border">
          <button
            onClick={() => onQuickEdit('')}
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold text-white transition-all hover:opacity-90"
            style={{ background: 'linear-gradient(135deg, #7C5CFC, #5A3DE8)' }}
          >
            <Wand2 size={11} /> Continue editing with AI
          </button>
        </div>
      )}
    </>
